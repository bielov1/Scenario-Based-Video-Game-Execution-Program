...
{
	double dx = p2.x - p1.x; // ray dirx
	double dy = p2.y - p1.y; // ray diry
...
void Raycaster::draw_frame(WorldMap& world_map, Pixel* pixels, Player& player, int screen_width, int screen_height)
{
	...
	double render_width = screen_width * 0.5;
	double strip_width = screen_width / render_width;

	for (int x = 0; x < render_width; x++) {
		Vector2 p = p1.lerp(p2, x / render_width);
		Ray_Hit ray_hit = cast_ray(world_map, player.pos, p, (int)grid_size.x, (int)grid_size.y);

		if (ray_hit.hit) {
			COLORREF rgb = world_map.map[(int)ray_hit.hit_pos.y][(int)ray_hit.hit_pos.x].get_rgb_color(); 
			...
			double perp_dist = ray_hit.ray_len * ray_dir.dot(cam_dir);
